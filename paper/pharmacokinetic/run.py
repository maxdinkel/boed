...
if __name__ == '__main__':
    ...
    for i in range(4):
        results["nmc"].append(repeat_algorithm(SDC_(estimator=nmc, num_eval=nmc.compute_num_total([32, 40, 50, 64][i]))))
        results["rnmc"].append(repeat_algorithm(SDC_(estimator=rnmc, num_eval=200 * 2 ** i)))
        results["lrnmc"].append(repeat_algorithm(SDC_(estimator=lrnmc, num_eval=200 * 2 ** i)))
        results["anmc"].append(repeat_algorithm(SDC_(estimator=anmc, num_eval=200 * 2 ** i)))
    ...
    for i in range(4):
        results["nmc"].append(repeat_algorithm(SDC_(estimator=nmc, num_eval=nmc.compute_num_total([16, 20, 25, 32][i]))))
        results["rnmc"].append(repeat_algorithm(SDC_(estimator=rnmc, num_eval=100 * 2 ** i)))
        results["lrnmc"].append(repeat_algorithm(SDC_(estimator=lrnmc, num_eval=100 * 2 ** i)))
    ...
    for num_criterion in [2, 3]:
        for num_new_eval in [8, 16]:
            for q in [0.95, 0.99, 0.999]:
                ...
                print(f'nc{num_criterion}_q{str(q).replace("0.", "")}_n{num_new_eval}')
